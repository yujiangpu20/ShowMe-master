...
class DDPM(pl.LightningModule):
    # classic DDPM with Gaussian diffusion, in image (pixel) space
    def __init__(self,
                 unet_config,
                 timesteps=1000,
        ...
        self.model = DiffusionWrapper(unet_config, conditioning_key)  # 3D U-Net
        ...
        self.use_scheduler = scheduler_config is not None  # LR scheduler
        if self.use_scheduler:
            self.scheduler_config = scheduler_config

        self.v_posterior = v_posterior
        self.original_elbo_weight = original_elbo_weight
        self.l_simple_weight = l_simple_weight  # loss weight
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        # calculations for posterior q(x_{t-1} | x_t, x_0) = N(x_{t-1} | mean, variance)
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.final_frame_prediction = final_frame_prediction
        self.reward_tuning = reward_tuning
        self.reward_type = reward_type
        self.en_and_decode_n_samples_a_time = en_and_decode_n_samples_a_time  # not use
        ...
        self.instantiate_first_stage(first_stage_config)  # VAE initialization
        self.instantiate_cond_stage(cond_stage_config)  # CLIP Image Conditioner initialization
    ...
    def structure_reward_loss(self, x_noisy, t, model_output, th=200, **kwargs):
        if self.parameterization == "eps":
            x_recon = self.predict_start_from_noise(x_noisy, t=t, noise=model_output)
        elif self.parameterization == "x0":
            x_recon = model_output

        indices = torch.where(t <= th)[0]
        if len(indices) == 0:
            return torch.tensor(0., device=self.device)
        else:
            valid_x_recon = x_recon[indices]  # (n, c, t, h, w)
            x_sample = self.differentiable_decode_first_stage(valid_x_recon[:, :, [-1], ...])  # last frame
            x_sample = x_sample.squeeze(2)  # (n, c, h, w)

            all_x = kwargs.get('ori_input', None)  # get full video input
            x_tgt = all_x[:, :, -1][indices]  # (n, c, 256, 256)
            # x_cond = all_x[:, :, 0][indices]  # (n, c, 256, 256)

            reward_loss = torch.tensor(0., device=self.device)
            for reward in self.reward_type:
                if reward == 'edge':
                    reward_loss += self._edge_loss(x_sample, x_tgt)
                elif reward == 'depth':
                    reward_loss += self._depth_loss(x_sample, x_tgt)
                else:
                    raise NotImplementedError(f"Reward type {reward} not yet supported")

        return reward_loss

    def motion_loss(self, x_noisy, t, model_output, th=700, eps=1e-6, alpha=1e-2, **kwargs):
        """
        Compute KL divergence between the temporal distribution of pixel-space flow magnitudes
        and latent-space difference magnitudes at each spatial location.
        """
        if self.parameterization == "eps":
            x_pred = self.predict_start_from_noise(x_noisy, t=t, noise=model_output)
        else:
            x_pred = model_output

        indices = torch.where(t >= th)[0]  # choose noiser samples
        if len(indices) == 0:
            return torch.tensor(0., device=self.device)

        # x_sample = x_pred[indices]
        x_sample = x_pred
        gt_video = kwargs.get('ori_input', None)
        # v_sample = gt_video[indices]
        v_sample = gt_video

        b, C, T, h, w = x_sample.shape
        _, _, _, H, W = v_sample.shape

        # # 1) Pixel-space flow magnitude sequence
        v_sample = (v_sample + 1) / 2  # normalize to [0, 1]
        st = rearrange(v_sample[:, :, :-1], 'b c t H W -> (b t) c H W')
        ed = rearrange(v_sample[:, :, 1:], 'b c t H W -> (b t) c H W')
        raft_model = self.reward_models.raft_model.to(self.device)
        with torch.no_grad():
            flow = raft_model(st, ed)[-1]
        flow = flow.view(b, T-1, 2, H, W)

        mag_flow = flow.norm(dim=2)  # (b, T-1, H, W)

        # 2) Downsample spatial dims to latent resolution
        mag_flow_flat = mag_flow.reshape(b * (T - 1), 1, H, W)
        mag_flow_ds = F.adaptive_avg_pool2d(mag_flow_flat, (h, w)).reshape(b, T - 1, h, w)

        # 3) Latent-space difference magnitude sequence
        du = x_sample[:, :, 1:] - x_sample[:, :, :-1]  # (b, C, T-1, h, w)
        mag_latent = du.norm(dim=1)  # (b, T-1, h, w)

        # 4) Build temporal distributions per pixel
        P = mag_flow_ds + eps
        P = P / P.sum(dim=1, keepdim=True)  # normalize over time

        Q = mag_latent + eps
        Q = Q / Q.sum(dim=1, keepdim=True)

        # 5) KL divergence per spatial location; Sum over time, yielding (b, h, w)
        kl_map = (P * (P.log() - Q.log())).sum(dim=1)
        loss = alpha * kl_map.mean()

        return loss

    def _edge_loss(self, x, x_gt):
        hedge_model = self.reward_models.hedge_model.to(self.device)
        x = (x + 1) / 2
        x = F.interpolate(x, size=(320, 480), mode="bicubic", align_corners=False)
        x_gt = (x_gt + 1) / 2
        x_gt = F.interpolate(x_gt, size=(320, 480), mode="bicubic", align_corners=False)

        with torch.cuda.amp.autocast(enabled=False):
            x = x.float()
            x_gt = x_gt.float()
            with torch.no_grad():
                x_edge = hedge_model(x)
                x_gt_edge = hedge_model(x_gt)

        return F.mse_loss(x_edge, x_gt_edge, reduction='mean')

    def _depth_loss(self, x_sample, x_tgt):
        ds_model = self.reward_models.ds_model.to(self.device)
        sample_depth = self._apply_depth_model(ds_model, x_sample)
        tgt_depth = self._apply_depth_model(ds_model, x_tgt)
        return F.mse_loss(sample_depth, tgt_depth, reduction='mean')

    def _apply_depth_model(self, ds_model, x):
        b, c, h, w = x.shape
        x = (x + 1) / 2
        x = F.interpolate(x, size=(256, 256), mode="bicubic", align_corners=False)  # Only for MiDaS_small
        transform = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        x = transform(x)  # [B,3,H,W]

        with torch.no_grad():
            prediction = ds_model(x)  # => [b, H', W'] or [b,1,H',W']
    ...
    def p_losses(self, x_start, cond, t, noise=None, **kwargs):
        ...
        if self.final_frame_prediction:
            kwargs.update({'final_frame_prediction': True})  # disable TemporalTransformer
        ...
        if self.reward_tuning and self.reward_models is not None:
            if self.final_frame_prediction:
                reward_loss = self.structure_reward_loss(x_noisy, t, model_output, **kwargs)
            else:
                reward_loss = self.motion_loss(x_noisy, t, model_output, **kwargs)

            if reward_loss is not None:
                loss_dict[f'{prefix}/reward_loss'] = reward_loss
                loss += reward_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #     loss_dict_ema = {key + '_ema': loss_dict_ema[key] for key in loss_dict_ema}
        #
        # val_loss = loss_dict_no_ema['val/loss']
        # self.log('val_loss', val_loss, prog_bar=True, logger=True, on_epoch=True, sync_dist=True)
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        if ddim:
            ...
            if self.final_frame_prediction:
                shape = (self.channels, 1, *self.image_size)  # for single frame generation
            else:
    ...
    def get_lora_state_dict(
        ...
        """
        Return a state_dict that only contains LoRA parameters.

        Args:
            include_ema:
                - False -> only LoRA params from the main model / image_proj
                - True  -> also include LoRA params inside model_ema (if present)
        """
        ...
        for name, tensor in full_sd.items():
            ...
            # 1) keep only LoRA params
            if "lora" not in lower:  # covers lora_A, lora_B, etc.
                continue

            # 2) optionally skip EMA buffers
            if not include_ema and "model_ema" in name:
                continue

            # 3) optionally stage-filter
            if stage_tag is not None and stage_tag not in lower:
                continue

            lora_sd[name] = tensor.cpu()
    ...
    def load_lora_state_dict(
        ...
        """
        Load LoRA weights on top of the current base model.
        """
        # Optionally filter out EMA keys if not desired
        if not include_ema:
            lora_sd = {k: v for k, v in lora_sd.items() if "model_ema" not in k}

        # Safe with strict=False: non-LoRA weights untouched, missing keys ignored
        missing, unexpected = self.load_state_dict(lora_sd, strict=strict)
...
class LatentVisualDiffusion(LatentDiffusion):
    def __init__(self, img_cond_stage_config, image_proj_stage_config, freeze_embedder=True,
                 image_proj_model_trainable=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.image_proj_model_trainable = image_proj_model_trainable
        self._init_embedder(img_cond_stage_config, freeze_embedder)  # CLIP Image Embedder
        self._init_img_ctx_projector(image_proj_stage_config, image_proj_model_trainable)  # Q-Former
    ...
    def _init_img_ctx_projector(self, config, trainable):
        ...
        if trainable:
            if config.params.get("tuning_type", None) == 'lora':
                current_stage = config.params.get("current_lora_stage", 1)
                for name, param in self.image_proj_model.named_parameters():
                    if 'lora' in name.lower():
                        if current_stage == 1:
                            param.requires_grad = ('stage1' in name.lower())
                        else:
                            param.requires_grad = ('stage2' in name.lower())
            else:
                for param in self.image_proj_model.parameters():
    ...
    def shared_step(self, batch, random_uncond, **kwargs):
        # Disable classifier-free guidance sampling if reward tuning is enabled
        if self.reward_tuning:
            random_uncond = False

        x, c, xc, fs, all_x = self.get_batch_input(
            batch,
        ...
        if self.reward_tuning:
            kwargs.update({
                "txt_cond": xc,  # Original text prompt
                "ori_input": all_x  # Original input
            })
    ...
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False,
                        return_fs=False, return_cond_frame=False, return_original_input=False, **kwargs):

        x = super().get_input(batch, self.first_stage_key)  # (b, c, t, h, w)

        if self.final_frame_prediction:
            x = self.get_ffp_input(x)  # b c 2 h w
        ...
        img = input_mask * img  # random drop img condition
        ## img: b c h w
        ...
        if self.model.conditioning_key == 'hybrid':
            if self.final_frame_prediction:
                img_cat_cond = z[:, :, [cond_frame_index], :, :]  # initial frame condition
                z = z[:, :, [-1], :, :]  # target final frame

            else:
                img_cat_cond = torch.zeros_like(z, device=z.device)
                img_cat_cond[:, :, 0, :, :] = z[:, :, 0, :, :]  # first frame cond
                img_cat_cond[:, :, -1, :, :] = z[:, :, 0, :, :]

                drop_decisions = torch.rand(z.size(0), device=z.device) < 5 * self.uncond_prob  # drop 25%
                img_cat_cond[:, :, -1, :, :] = torch.where(drop_decisions.view(-1, 1, 1, 1),
                                                           torch.zeros_like(img_cat_cond[:, :, -1, :, :]),
                                                           z[:, :, 0, :, :])
        ...
        out = [z, cond]
        if return_first_stage_outputs:
            xrec = self.decode_first_stage(z)  # target
            out.extend([xrec])
    ...
    def get_ffp_input(self, x, cond_frame_num=2, target_frame_num=3):
        b, c, t, h, w = x.shape

        # Generate indices for condition frames and target frames
        cond_n_indices = torch.randint(0, cond_frame_num, (b,))
        last_n_indices = torch.randint(t - target_frame_num, t, (b,))

        # Gather the condition frames and target frames in one step using advanced indexing
        cond_frame_batch = x[torch.arange(b), :, cond_n_indices, :, :].unsqueeze(2)
        target_frame_batch = x[torch.arange(b), :, last_n_indices, :, :].unsqueeze(2)

        # Concatenate the condition frames and target frames along the temporal dimension
        input_x = torch.cat([cond_frame_batch, target_frame_batch], dim=2)

        return input_x

    def get_latent_zero_mask(self, z):
        b, c, t, h, w = z.shape
        mask_z = torch.zeros((b, c, t - 1, h, w)).to(z.device)
        return mask_z
    ...
    def log_images(self, batch, sample=True, ddim_steps=50, ddim_eta=1., plot_denoise_rows=False, \
        ...
        N = xrec.shape[0]
        log["image_condition"] = cond_x  # condition frame
        log["reconst"] = xrec  # reconstructed target frame
        xc_with_fs = []
        ...
        if sample:
            ...
            if self.final_frame_prediction:
                kwargs.update({'final_frame_prediction': True})  # disable TemporalTransformer (very important!!)
    ...
    def configure_optimizers(self):
        ...
        # params = list(self.model.parameters())
        params = [p for p in self.model.parameters() if p.requires_grad]  # only feed trainable params
        mainlogger.info(f"@Training [{len(params)}] Paramters.")

        if self.cond_stage_trainable:
            params_cond_stage = [p for p in self.cond_stage_model.parameters() if p.requires_grad == True]
            mainlogger.info(f"@Training Cond_stage_model: [{len(params_cond_stage)}] Paramters.")
            params.extend(params_cond_stage)

        if self.image_proj_model_trainable:
            params_image_proj = [p for p in self.image_proj_model.parameters() if p.requires_grad == True]
            mainlogger.info(f"@Training Image_proj_model: [{len(params_image_proj)}] Paramters.")
            params.extend(params_image_proj)
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        self._set_tuning_mode(diff_model_config)

    def _set_tuning_mode(self, config):
        tuning_type = config["params"].get("tuning_type", None)
        handlers = {
            'lora': lambda: self._lora_tuning(self.diffusion_model, config)
        }
        handler = handlers.get(tuning_type, lambda: None)
        handler()

    def _lora_tuning(self, model, config):
        current_stage = config["params"].get("current_lora_stage", 1)
        for name, param in model.named_parameters():
            if 'lora' in name.lower():
                if current_stage == 2:
                    # Stage 2 training: enable grads only for stage2 lora params
                    param.requires_grad = ('stage2' in name.lower())
                else:
                    # Stage 1 training: enable grads only for stage1 lora params
                    param.requires_grad = ('stage1' in name.lower())
            else:
                param.requires_grad = False
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None,
        ...
        if self.conditioning_key is None:
            ...
            xc = torch.cat([x] + c_concat, dim=1)  # x: noisy target  c_concat: clean latent condition
            cc = torch.cat(c_crossattn, 1)  # [clip_txt_embed, clip_img_embed]

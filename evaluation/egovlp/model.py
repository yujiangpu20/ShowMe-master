...
from .util import state_dict_data_parallel_fix
import sys
import types

if "parse_config" not in sys.modules:
    dummy_mod = types.ModuleType("parse_config")
    ...
    dummy_mod.ConfigParser = ConfigParser
    dummy_mod.parse_config = parse_config

    sys.modules["parse_config"] = dummy_mod
...
class FrozenInTime(BaseModel):
    def __init__(self,
        ...
        if self.text_params['model'].startswith('distilbert'):
            self.text_model = AutoModel.from_pretrained('distilbert-base-uncased', cache_dir='egovlp/pretrained/distilbert-base-uncased')
        else:
        ...
        if video_params['model'] == "SpaceTimeTransformer":
            ...
            if arch_config == 'base_patch16_224':
                ...
                vit_model = torch.load("egovlp/pretrained/jx_vit_base_p16_224-80ecf9dd.pth", map_location="cpu")
        ...
        if load_checkpoint not in ["", None]:
            checkpoint = torch.load(load_checkpoint)  # for debug -- add by Bolin, the original codes are for multi-gpu loading
    ...
    def compute_text_tokens(self, text_data):
        if self.text_params['model'].startswith('bert'):
            text_embeddings = self.text_model(text_data['input_ids'], attention_mask=text_data['attention_mask'])['pooler_output']    # not implement for bert
        elif self.text_params['model'].startswith('distilbert'):

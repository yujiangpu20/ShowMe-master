...
class ImageLogger(Callback):
    ...
    def log_batch_imgs(self, pl_module, batch, batch_idx, split="train"):
        ...
        skip_freq = self.batch_freq if split == "train" else 1  # set 5 for full validation set
        if (batch_idx+1) % skip_freq == 0:
            ...
            self.log_to_tensorboard(pl_module, batch_logs, filename, split, save_fps=6)  # log on tensorboard
            if self.to_local:
                mainlogger.info("Log [%s] batch <%s> to local ..."%(split, filename))
                filename = "gs{}_".format(pl_module.global_step) + filename
                log_local(batch_logs, os.path.join(self.save_dir, split), filename, save_fps=6)  # save to local path
...
class LoRACheckpoint(Callback):
    def __init__(self, every_n_train_steps=1000, save_dir=None, prefix="lora"):
        super().__init__()
    ...
    def on_fit_start(self, trainer, pl_module):
        # If no save_dir is provided, default to <model.logdir>/checkpoints_lora
        if self.save_dir is None:
            base_dir = getattr(pl_module, "logdir", None)
            if base_dir is None:
                base_dir = trainer.default_root_dir
            self.save_dir = os.path.join(base_dir, "checkpoints_lora")
        if trainer.is_global_zero:
            os.makedirs(self.save_dir, exist_ok=True)

    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        # Use trainer.global_step (0-based) as your step index
        ...
        # IMPORTANT: LoRA helpers live on the *inner* diffusion model
        # (LatentDiffusion / LatentVisualDiffusion), not on DiffusionWrapper.
        diffusion_model = getattr(pl_module, "diffusion_model", pl_module)

        if not hasattr(diffusion_model, "get_lora_state_dict"):
            # Safety guard: do nothing if helpers are missing
            return

        lora_sd = diffusion_model.get_lora_state_dict()
        filename = f"{self.prefix}_step{global_step}.pth"
        ckpt_path = os.path.join(self.save_dir, filename)
        torch.save(lora_sd, ckpt_path)
        trainer.logger.log_metrics(
            {"lora_ckpt_saved": float(global_step)}, step=global_step
        )
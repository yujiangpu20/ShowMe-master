...
def get_fid_logits(videos, i3d, device, target_resolution=224):
    videos = preprocess(videos, target_resolution=target_resolution)
    embeddings = get_logits(i3d, videos, device)
    return embeddings
...
@torch.no_grad()
def eval_video_fvd(device, i3d, pred, gt, fake_embeddings_stack, real_embeddings_stack):
    fake = pred  # BTHWC, 'uint8'
    fake_embeddings_stack.append(get_fvd_logits(fake, i3d=i3d, device=device))
    real = gt  # BTHWC, 'uint8'
    real_embeddings_stack.append(get_fvd_logits(real, i3d=i3d, device=device))
...
# pred = np.random.randn(32, 400)
# gt = np.random.randn(32, 400)
# kvd = polynomial_mmd(pred, pred)
# print(kvd)

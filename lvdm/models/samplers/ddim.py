...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      fs=None, guidance_rescale=0.0, **kwargs):
        device = self.model.betas.device
        b = shape[0]
        if x_T is None:
            img = torch.randn(shape, device=device)  # sample random gaussian noise
        else:
            img = x_T
        ...
        for i, step in enumerate(iterator):
            ...
            # use mask to blend sampled noisy latent (img) & original latent (img_orig)
            if mask is not None:

...
#                 continue  # skip temporal transformer for FFP
#             else:
#                 x = layer(x)
#         return x
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    support it as an extra input.
    """
    def forward(self, x, emb, context=None, batch_size=None, final_frame_prediction=False):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb, batch_size=batch_size)
            elif isinstance(layer, SpatialTransformer):
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fs=None, **kwargs):
        ...
        if (l_context - 77) % t == 0:  ## !!! HARD CODE here
        ...
        self.ffp = kwargs.pop('final_frame_prediction', False)
        if not self.ffp:
            ## combine emb
        ...
        for id, module in enumerate(self.input_blocks):
            h = module(h, emb, context=context, batch_size=b, final_frame_prediction=self.ffp)
            if id == 0 and self.addition_attention:
                h = self.init_attn(h, emb, context=context, batch_size=b, final_frame_prediction=self.ffp)
        ...
        h = self.middle_block(h, emb, context=context, batch_size=b, final_frame_prediction=self.ffp)
